...
let search_Url=`https://api.spoonacular.com/recipes/complexSearch?query=${name}&maxFat=25&addRecipeInformation=true&apiKey=${Api_key}`;
 console.log(name)
...
async function getcategorywise(){
  ...
  async function searchitem(){
    let output=await fetch(search_Url);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5  max-w-fit mx-auto "   >
      {
        recipe.map((item,index)=>(
          ...
          <div className="p-2">
            ...
            dangerouslySetInnerHTML={{ __html: item.summary||item.instructions }}
          />
          </div>
